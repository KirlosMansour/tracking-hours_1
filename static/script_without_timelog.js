...
/* Force-commit any value that the user has typed into an input but hasn't
   clicked away from yet.  Without this, clicking the Combine/Split button
   while an input still has focus causes the snapshot to capture the OLD
   (pre-edit) value from STATE instead of what's visible on screen. */
...
function toggleCombineCells(group) {
  ...
  // otherwise values the user just typed (but hasn't blurred from) are
  // still only in the DOM, not in STATE, and the snapshot saves zeros.
  ...
  if (turningOn) {
    ...
    // On split: restore BAC, PCT, ETC from the snapshot.
    // For AC: any hours that were added via the Time Log while in combined
    // mode are stored on r1.  We put those back proportionally — Drawings
    // gets its original share, Studies gets the rest (which includes any
    // newly-logged hours so nothing is lost).
    const snap = project.combineSnapshots[group];
    if (snap) {
      const currentAc   = Number(r1.ac) || 0;
      const snapAcTotal = (Number(snap.ac1) || 0) + (Number(snap.ac2) || 0);
      const extraAc     = currentAc - snapAcTotal;  // hours logged while combined

      r1.bac = snap.bac1; r1.pct = snap.pct1; r1.etc = snap.etc1;
      r2.bac = snap.bac2; r2.pct = snap.pct2; r2.etc = snap.etc2;

      // Distribute AC: original amounts go back to their rows, any extra
      // (from time-log entries while combined) goes to the first row
      // (Design Drawings) since that's where they were logged.
      r1.ac = (Number(snap.ac1) || 0) + Math.max(0, extraAc);
      r2.ac =  Number(snap.ac2) || 0;

      delete project.combineSnapshots[group];
      toast(`${group}: rows split — extra logged hours kept on "${r1.task}"`);
    } else {
