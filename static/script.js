...
function toggleCombineCells(group) {
  ...
  if (turningOn) {
    ...
    if (snap) {
      // Restore BAC, PCT, ETC from snapshot.
      // For AC: any hours added via Time Log while combined are on r1 —
      // keep them on the first row (Design Drawings) so nothing is lost.
      ...
      toast(`${group}: rows split — logged hours kept on "${r1.task}"`);
    } else {
