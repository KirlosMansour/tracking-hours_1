...
function updateAllNextEstimates() {
  ...
  document.querySelectorAll("#phasesBody tr[data-row-idx]").forEach(tr => {
    ...
    if (!cell) return;
    const c = calcRow(row);
    cell.textContent = fmt0(c.eacMain * (1 + buf));
  });
}
/* ---------------------------------------------------------------------- */
...
function renderSummaryCards(project) {
  ...
  const totalWDays   = workingDaysBetween(project.startDate, project.endDate);   // planned total (no weekends)
  const elapsedWDays = workingDaysBetween(project.startDate, today);             // actual elapsed (no weekends)
  const daysLeft     = workingDaysBetween(today, project.endDate);               // working days still to planned end

  // Planned hrs/day = BAC ÷ total planned working days
  const plannedHrsDay = (totalWDays && t.totalBac > 0)
    ? (t.totalBac / totalWDays).toFixed(1) : null;

  // Actual hrs/day = AC ÷ elapsed working days (how fast are we actually burning hours)
  const actualHrsDay = (elapsedWDays && elapsedWDays > 0 && t.totalAc > 0)
    ? (t.totalAc / elapsedWDays).toFixed(1) : null;

  // Expected end date based on actual burn rate:
  //   remaining hours (from EAC) = EAC - AC
  //   working days needed = remaining ÷ actual_hrs_per_day
  //   expected end = today + those working days (skipping weekends)
  let expectedEnd = null, expectedVsPlanned = null;
  if (actualHrsDay && Number(actualHrsDay) > 0 && t.totalEac > t.totalAc) {
    const remainingHrs  = t.totalEac - t.totalAc;
    const daysNeeded    = remainingHrs / Number(actualHrsDay);
    expectedEnd         = addWorkingDays(today, daysNeeded);

    // Compare expected vs planned (negative = early, positive = late)
    if (expectedEnd && project.endDate) {
      ...
      expectedVsPlanned = expectedEnd <= project.endDate
        ? { days: diff, early: true }
        : { days: diff, early: false };
    }
  } else if (t.totalEac <= t.totalAc && t.totalAc > 0) {
    // Work appears done (AC >= EAC)
    expectedEnd = today;
  }
  ...
  const dateCards = (project.startDate || project.endDate) ? `
    ...
    ${expectedEnd ? `
    <div class="card ${expectedVsPlanned ? (expectedVsPlanned.early ? "card-good" : "card-warn") : ""}">
      <div class="label">Expected End (Actual Pace)</div>
      <div class="value" style="font-size:14px;color:${expectedVsPlanned ? (expectedVsPlanned.early ? "var(--green)" : "var(--red)") : "inherit"}">
        ${expectedEnd}
        ${expectedVsPlanned ? `<span class="card-sub" style="color:${expectedVsPlanned.early ? "var(--green)" : "var(--red)"}">
          ${expectedVsPlanned.early ? `▲ ${expectedVsPlanned.days} days early` : `▼ ${expectedVsPlanned.days} days late`}
        </span>` : ""}
      </div>
    </div>` : ""}
  ` : "";
