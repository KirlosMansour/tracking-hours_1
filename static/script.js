...
function switchTeamPanel(which) {
  // which = "workload" | "forecast"
  const wpanel = document.getElementById("teamWorkloadPanel");
  const fpanel = document.getElementById("teamForecastPanel");
  const wbtn   = document.getElementById("btnViewWorkload");
  const fbtn   = document.getElementById("btnViewForecast");
  if (!wpanel || !fpanel || !wbtn || !fbtn) return;

  const showWorkload = which === "workload";
  wpanel.classList.toggle("hidden", !showWorkload);
  fpanel.classList.toggle("hidden",  showWorkload);
  wbtn.classList.toggle("active",    showWorkload);
  fbtn.classList.toggle("active",   !showWorkload);
}
...
function renderTeamWorkloadView() {
  ...
  TEAM_MEMBERS.forEach(member => {
    ...
    card.innerHTML = `
      ...
      ${buildAvailabilityHTML(member, w.available)}`;
...
function refreshApplyLogSummary() {
  ...
  // Group entries by phaseRowId
  const byPhase = {};
  (STATE.timeLogs || [])
    .filter(e => e.projectId === projectId)
    .forEach(e => {
      const key = e.phaseRowId;
      if (!byPhase[key]) byPhase[key] = { hours: 0, count: 0, label: e.type || "—" };
      byPhase[key].hours += Number(e.hours || 0);
      byPhase[key].count += 1;
    });
  ...
  const totalHrs   = Object.values(byPhase).reduce((s, v) => s + v.hours, 0);
  const totalEntries = Object.values(byPhase).reduce((s, v) => s + v.count, 0);

  const rows = project.phases
    .filter(r => byPhase[r.id])
    .map(r => {
      const info = byPhase[r.id];
      const curAc = Number(r.ac) || 0;
      return `<tr>
        <td>${escapeHtml(r.group)}</td>
        <td>${escapeHtml(r.task)}</td>
        <td class="num">${curAc}</td>
        <td class="num arrow-col">→</td>
        <td class="num new-val">${info.hours.toFixed(1)}</td>
  ...
  wrap.innerHTML = `
    <div class="apply-log-summary-inner">
      <p class="apply-log-meta">${totalEntries} log entries · ${totalHrs.toFixed(1)} total hrs across ${Object.keys(byPhase).length} phase rows</p>
      <table class="apply-log-table">
        <thead><tr>
          <th>Stage</th><th>Task</th>
          <th>Current AC</th><th></th>
          <th>New AC</th><th>Entries</th>
        </tr></thead>
        <tbody>${rows}</tbody>
      </table>
      <p class="apply-log-warn">⚠ Current AC values will be replaced by the Time Log totals shown above.</p>
    </div>`;
}

/** Apply all Time Log entries for a project to its phase AC values. */
function applyTimeLogToProject() {
  ...
  // Count total entries and hours for confirmation message
  ...
  const msg =
    ...
    `${entries.length} entries totalling ${totalHrs.toFixed(1)} hrs will be used to\n` +
    `SET the Actual Hours (AC) for each phase.\n\n` +
    `⚠ Current manual AC values will be overwritten.\n\n` +
    `Press OK to apply, Cancel to go back.`;

  if (!confirm(msg)) return;

  // Group by phaseRowId and SET (not increment) each phase's AC
  const byPhase = {};
  entries.forEach(e => {
  ...
  project.phases.forEach(row => {
    if (byPhase[row.id] !== undefined) {
      row.ac = Math.round(byPhase[row.id] * 10) / 10;
    }
  });
  ...
  refreshDashboardRow(project);
  renderTimeLogView();
  toast(`✅ Applied ${entries.length} entries (${totalHrs.toFixed(1)} hrs) to "${project.name}"`);
}
