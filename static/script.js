...
function makeProject(name) {
  return {
    ...
    startDate:       "",
    endDate:         "",
    teamAllocations: [],   // [{member:"Sriram", hours:300}, ...] — person-level hour breakdown
  };
}
...
function renderStageStatus(project) {
  ...
  // Expected End chip — calculated from actual burn rate, placed to the right
  let expectedEndChip = "";
  if (project.startDate && project.phases.some(r => Number(r.ac) > 0)) {
    ...
    const actualHrsDay = (elapsedWDays && elapsedWDays > 0)
      ? pt.totalAc / elapsedWDays : 0;

    if (actualHrsDay > 0 && pt.totalEac > pt.totalAc) {
      const remaining  = pt.totalEac - pt.totalAc;
...
function upsertAllocation(project, member, hours) {
  ...
  else          { project.teamAllocations.push({ member, hours }); }
  saveState();
  renderTeamAllocationSection(project);
}
...
function renderTeamAllocationSection(project) {
  ...
  const rows = allocs.length === 0
    ? `<tr><td colspan="5" style="text-align:center;color:var(--text-muted);padding:16px">No allocations yet — add a team member above</td></tr>`
    : allocs.map(a => {
        const hrs     = Number(a.hours) || 0;
        const pct     = total > 0 ? (hrs / total * 100).toFixed(1) : "0.0";
        const hpd     = wDays  ? (hrs / wDays).toFixed(2) : "—";
        return `<tr>
          ...
          <td class="num"><input type="number" min="0" step="1" class="cell-input alloc-input"
              data-member="${escapeHtml(a.member)}" value="${hrs}" style="width:80px"></td>
          <td class="num">${pct}%</td>
          <td class="num">${hpd}${wDays ? " hrs/day" : ""}</td>
  ...
  wrap.innerHTML = `
    ...
    <table class="alloc-table">
      <thead><tr>
        <th>Team Member</th><th>Allocated Hours</th><th>Share %</th>
        <th>Hrs / Day${wDays ? `<span class="sub"> (${wDays} working days)</span>` : ""}</th>
        <th></th>
      </tr></thead>
      <tbody>${rows}</tbody>
      <tfoot><tr>
  ...
  wrap.querySelectorAll(".alloc-input").forEach(inp => {
    inp.addEventListener("blur", e => {
      ...
      if (hrs >= 0) upsertAllocation(project, e.target.dataset.member, hrs);
    });
  });
...
function getMemberWorkloadOnDate(memberName, date) {
  ...
  STATE.projects.forEach(p => {
    ...
    const memberPct = hrs / total; // e.g. 0.35 for 35%

    // Project is "active" on this date only if it has dates AND date is within range.
    // If no dates set, show it as always active.
    const withinRange = (!p.startDate && !p.endDate)
      ...
      || (!p.startDate && p.endDate && date <= p.endDate);

    if (!withinRange) return;

    // Member daily commitment = their hours / project working days.
    // If no dates, use a placeholder (we'll show ? hrs/day).
    const wDays    = workingDaysBetween(p.startDate, p.endDate);
    const hrsPerDay = wDays ? hrs / wDays : null;
...
function renderTeamWorkloadView() {
  ...
  TEAM_MEMBERS.forEach(member => {
    ...
    const projectRows = w.commitments.length === 0
      ...
      : w.commitments.map(c => `
          <div class="wl-proj-row">
            <div class="wl-proj-name">${escapeHtml(c.project.name)}</div>
            <div class="wl-proj-meta">
              <span class="wl-badge">${c.pct.toFixed(1)}% share</span>
              <span class="wl-hrs">${c.hrsPerDay !== null ? c.hrsPerDay.toFixed(2) + " hrs/day" : "— hrs/day"}</span>
              <span class="wl-range">${c.project.startDate || "?"} → ${c.project.endDate || "?"}</span>
            </div>
          </div>`).join("");
