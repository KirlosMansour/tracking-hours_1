...
function renderSummaryCards(project) {
  ...
  const hPerDay = (wDays && t.totalBac > 0) ? (t.totalBac / wDays).toFixed(1) : null;
  const today   = new Date().toISOString().split("T")[0];
  const daysLeft = project.endDate ? workingDaysBetween(today, project.endDate) : null;
  ...
  const dateCards = (project.startDate || project.endDate) ? `
    ...
    <div class="card">
      <div class="label">Est. End Date</div>
      <div class="value" style="font-size:15px">${project.endDate || "—"}</div>
    </div>
    ${wDays !== null ? `
    <div class="card">
      <div class="label">Working Days</div>
      <div class="value">${wDays}${hPerDay ? `<span style="font-size:12px;font-weight:500;color:var(--text-muted);display:block">${hPerDay} hrs/day planned</span>` : ""}</div>
    </div>` : ""}
    ${daysLeft !== null ? `
    <div class="card ${daysLeft < 10 ? "card-warn" : ""}">
      <div class="label">Working Days Left</div>
      <div class="value" style="color:${daysLeft < 10 ? "var(--red)" : "inherit"}">${daysLeft >= 0 ? daysLeft : "Overdue"}</div>
    </div>` : ""}` : "";
