...
TEAM_MEMBERS = [
    ...
    "General",   # catch-all for hours not assigned to a specific person
]
...
def export_pdf():
    ...
    pcols = ["Group","Task","BAC","AC","%Comp","ETC","EV","CPI",
             "EAC-1","EAC-2","EAC-3","EAC(used)","Var","Next Est."]

    for p in projects:
        t        = project_totals(p)
        start    = p.get("startDate","")
        end_date = p.get("endDate","")
        mw       = float(p.get("mw") or 0)
        buf_pct  = float(p.get("bufferPct") or 20)
        is_closed = p.get("isClosed", False)
        status_txt = "CLOSED" if is_closed else t["status"]
        ...
        def _wdays(s, e):
            ...
            try:
                sd, ed = _dt.fromisoformat(s), _dt.fromisoformat(e)
                return sum(1 for d in (sd+_td(n) for n in range((ed-sd).days+1)) if d.weekday()<5)
            except: return None

        w_days_total   = _wdays(start, end_date)
        today_str      = _dt.today().isoformat()
        w_days_elapsed = _wdays(start, today_str)
        w_days_left    = _wdays(today_str, end_date)

        planned_hpd = f'{t["total_bac"]/w_days_total:.1f}' if w_days_total else "—"
        actual_hpd  = f'{t["total_ac"]/w_days_elapsed:.1f}' if (w_days_elapsed and t["total_ac"]>0) else "—"

        # --- Project header ---
        type_label = p.get("projectType","PV")
        mw_label   = f'{mw:.0f} MW {type_label}' if mw else type_label
        elements.append(Paragraph(
            f'<b>{p.get("name","Project")}</b> &nbsp;|&nbsp; {mw_label} &nbsp;|&nbsp; {status_txt}', h2))

        # --- KPI summary row ---
        kpi_data = [[
            "Total BAC", "Total AC", "% Complete", "Overall CPI",
            "Total EAC", "Variance", "Status"
        ],[
            f'{t["total_bac"]:.0f} hrs',
            f'{t["total_ac"]:.0f} hrs',
            f'{t["overall_pct"]:.1f}%',
            f'{t["overall_cpi"]:.2f}' if t["overall_cpi"] else "N/A",
            f'{t["total_eac"]:.0f} hrs',
            f'{t["total_variance"]:+.0f} hrs',
            status_txt,
        ]]
        kpi_tbl = Table(kpi_data, repeatRows=0)
        scol2 = {"On Track": colors.HexColor("#16a34a"), "Watch": colors.HexColor("#ca8a04"),
                 "Over Budget": colors.HexColor("#dc2626"), "CLOSED": colors.HexColor("#6b7280")}
        kpi_tbl.setStyle(TableStyle([
            ...
            ("TEXTCOLOR",(6,1),(6,1),scol2.get(t["status"],colors.black)),
        ]))
        elements += [kpi_tbl, Spacer(1,6)]

        # --- Schedule + MW row ---
        sched_rows = [["Start Date","Planned End","Working Days","Planned hrs/day",
                        "Elapsed Days","Actual hrs/day","Days Left",
                        "Planned hrs/MW","Actual hrs/MW"]]
        sched_rows.append([
            start or "—", end_date or "—",
            str(w_days_total) if w_days_total else "—",
            planned_hpd,
            str(w_days_elapsed) if w_days_elapsed else "—",
            actual_hpd,
            str(w_days_left) if w_days_left is not None else "—",
            f'{t["total_bac"]/mw:.1f}' if mw else "—",
            f'{t["total_ac"]/mw:.1f}' if (mw and t["total_ac"]) else "—",
        ])
        stbl = Table(sched_rows, repeatRows=1)
        ...
        elements += [stbl, Spacer(1,8)]

        # --- Phase breakdown table ---
        elements.append(Paragraph("Phase Breakdown", h2))
        td = [pcols]
        for r in p.get("phases", []):
            c       = calc_row(r)
            next_e  = c["eac_main"] * (1 + buf_pct/100)
            td.append([r.get("group",""), r.get("task",""),
                       ...
                       f'{next_e:.0f}'])
        # totals footer row
        total_next = sum(calc_row(r)["eac_main"] for r in p.get("phases",[])) * (1+buf_pct/100)
        td.append(["TOTAL","",
                   f'{t["total_bac"]:.0f}', f'{t["total_ac"]:.0f}',
                   f'{t["overall_pct"]:.1f}%', "",
                   f'{t["total_ev"]:.0f}',
                   f'{t["overall_cpi"]:.2f}' if t["overall_cpi"] else "N/A",
                   "","","",
                   f'{t["total_eac"]:.0f}', f'{t["total_variance"]:+.0f}',
                   f'{total_next:.0f} (+{buf_pct:.0f}%)'])
        ...
        ptbl.setStyle(TableStyle([
            ...
            ("FONTNAME",(0,0),(-1,0),"Helvetica-Bold"),
            ("GRID",(0,0),(-1,-1),0.4,colors.HexColor("#dddddd")),
            ("FONTSIZE",(0,0),(-1,-1),7),("ALIGN",(2,0),(-1,-1),"CENTER"),
            ("ROWBACKGROUNDS",(0,1),(-1,-2),[colors.white,colors.HexColor("#f5f7fa")]),
            ("FONTNAME",(0,-1),(-1,-1),"Helvetica-Bold"),
            ("BACKGROUND",(0,-1),(-1,-1),colors.HexColor("#D9E1F2")),
            # Next Est. column highlighted
            ("BACKGROUND",(-1,0),(-1,0),colors.HexColor("#163a5c")),
        ...
        if allocs:
            ...
            for a in allocs:
                ...
                hpd   = f'{hrs/w_days_total:.2f}' if w_days_total else "—"
                eff   = f'{hrs/w_days_total*wlp/100:.2f}' if w_days_total else "—"
            ...
            fc_rows   = [["Member","Planned","Actual","EV","CPI","EAC","Remaining","Variance"]]
            for a in allocs:
                member    = a.get("member","")
                bac_p     = float(a.get("hours",0) or 0)
                ac_p      = sum(float(e.get("hours",0)) for e in time_logs
                               if e.get("member")==member and e.get("projectId")==p.get("id",""))
                proj_pct  = t["overall_pct"] / 100
                ev_p      = bac_p * proj_pct
                cpi_p     = ev_p / ac_p if ac_p > 0 else None
                eac_p     = (bac_p / cpi_p) if cpi_p else (ac_p + (bac_p - ev_p))
                rem_p     = max(0, eac_p - ac_p)
                var_p     = eac_p - bac_p
                fc_rows.append([
                    member, f'{bac_p:.0f}', f'{ac_p:.0f}', f'{ev_p:.0f}',
                    f'{cpi_p:.2f}' if cpi_p else "N/A",
                    f'{eac_p:.0f}', f'{rem_p:.0f}', f'{var_p:+.0f}',
                ])
            # Resource gap
            gap = t["total_eac"] - total_hrs
            fc_rows.append(["Resource Gap","","","","","",
                            f'EAC {t["total_eac"]:.0f} vs {total_hrs:.0f} allocated',
                            f'{gap:+.0f} hrs'])

            elements.append(Spacer(1,8))
            elements.append(Paragraph("Team Forecasting", h2))
            ftbl = Table(fc_rows, repeatRows=1)
